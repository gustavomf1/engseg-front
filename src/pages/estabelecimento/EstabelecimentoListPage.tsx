    ...
    <div>
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
          ...
          <p className="text-slate-500">{busca ? 'Nenhum estabelecimento encontrado' : 'Nenhum estabelecimento cadastrado'}</p>
        </div>
      ) : (
        <>
                  ...
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button onClick={() => setDetalhes(item)} className="p-1.5 text-slate-500 hover:text-slate-800 hover:bg-gray-100 rounded" title="Ver detalhes"><Eye size={15} /></button>
                        {user?.perfil === 'ENGENHEIRO' && (
                          <>
                            <Link to={`/estabelecimentos/${item.id}/editar`} className="p-1.5 text-slate-500 hover:text-slate-800 hover:bg-gray-100 rounded"><Pencil size={15} /></Link>
                            {item.ativo ? (
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={() => setDetalhes(null)}>
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => setDetalhes(null)} className="text-slate-400 hover:text-slate-600 transition"><X size={20} /></button>
            </div>
            <div className="space-y-3">
              ...
              {(detalhes.logradouro || detalhes.numero) && (
                <div className="p-3 bg-slate-50 rounded-lg">
                  ...
                  <p className="text-sm font-medium text-slate-700">{[detalhes.logradouro, detalhes.numero].filter(Boolean).join(', nº ')}</p>
                </div>
              )}
              {detalhes.bairro && (
                <div className="p-3 bg-slate-50 rounded-lg">
              ...
              {(detalhes.cidade || detalhes.estado) && (
                <div className="p-3 bg-slate-50 rounded-lg">
                  ...
                  <p className="text-sm font-medium text-slate-700">{[detalhes.cidade, detalhes.estado].filter(Boolean).join(' / ')}</p>
                </div>
              )}
            ...
            <div className="flex justify-end mt-5">
              <button onClick={() => setDetalhes(null)} className="px-4 py-2 text-sm text-slate-600 hover:bg-gray-100 rounded-lg transition">Fechar</button>
            </div>
          </div>
        </div>
      )}
